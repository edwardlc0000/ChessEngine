...
void MoveGenerator::generate_queen_attacks(const ChessBoard& board)
{
	...
	while (white_queens.board)
	{
		int from = white_queens.get_LSB();
		white_queens.pop_LSB();
		for (int i = 0; i < 8; i++)
		{
			int to = from;
			int from_rank = to / 8;
			...
			while (true)
			{
				...
				if (board.bitboards[EMPTY].get_bit(to))
				{
					attacks[WHITE_QUEEN].set_bit(to);
				}
				else if (board.bitboards[BLACK_PIECES].get_bit(to))
				{
					attacks[WHITE_QUEEN].set_bit(to);
	...
	while (black_queens.board)
	{
		int from = black_queens.get_LSB();
		black_queens.pop_LSB();
		for (int i = 0; i < 8; i++)
		{
			int to = from;
			int from_rank = to / 8;
			...
			while (true)
			{
				...
				if (board.bitboards[EMPTY].get_bit(to))
				{
					attacks[BLACK_QUEEN].set_bit(to);
				}
				else if (board.bitboards[WHITE_PIECES].get_bit(to))
				{
					attacks[BLACK_QUEEN].set_bit(to);
